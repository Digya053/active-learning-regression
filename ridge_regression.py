...
def weight_regression(lambda_input, sigma2_input, X_train, y_train, dim):
    # wRR = (λI + X^TX)^−1X^Ty
    wRR = (np.linalg.inv(lambda_input * np.eye(dim) + \
           (X_train.T).dot(X_train)).dot((X_train.T).dot(y_train)))
    return wRR
...
def update_posterior(lambda_input, sigma2_input, data, label, dim):
    ...
    covariance = np.linalg.inv(
        lambda_input * np.eye(dim) + (1 / sigma2_input) * (data.T).dot(data))
    mean = np.linalg.inv((lambda_input *
                          sigma2_input *
                          np.eye(dim)) +
                         ((data.T).dot(data))).dot((data.T).dot(label))
    return covariance, mean


def active_learning(lambda_input, sigma2_input, X_train, y_train, X_test, dim):
    covariance, mean = update_posterior(
        lambda_input, sigma2_input, X_train, y_train, dim)
    wRR = mean
    ...
    for i in range(0, 10):
        ...
        covariance, mean = update_posterior(
            lambda_input, sigma2_input, data, label, dim)
        wRR = mean
...
def main():
    ...
    np.savetxt("active_" + str(lambda_input) + "_" +
               str(int(sigma2_input)) + ".csv", active_rows, delimiter=",")
